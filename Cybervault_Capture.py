...
model = keras.models.load_model('/CyberVault IDS Model/CyberVault IDS')

scaler = StandardScaler()
scaler.mean_ = np.load('/CyberVault IDS Model/scaler_mean.npy')
scaler.scale_ = np.load('/CyberVault IDS Model/scaler_scale.npy')
label_encoder = joblib.load('/CyberVault IDS Model/label_encoder.pkl')
...
def process_packet(packet):
    ...
    global df, model, label_encoder, scaler, total_fwd_length, total_bwd_length, fwd_lengths, bwd_lengths, start_time, total_bytes,
    last_pkt_time, total_pkts, last_pkt_count_time, fwd_IAT_total, bwd_IAT_total, SYN_flag_count,
    RST_flag_count, PSH_flag_count, ACK_flag_count, down_count, up_count, active_times, idle_times 
    ...
    if pkt_count_time - last_pkt_count_time > 1:  # Calculate over a 1-second interval
        flow_pkts_perSec = total_packets / (pkt_count_time - last_pkt_count_time)
