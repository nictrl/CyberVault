...
with open(csv_file, mode="w", newline="") as file:
    ...
    def start_capture():
        ...
        if not running:
            ...
            running = True
            subprocess.run(["iptables", "-t", "nat", "-A", "POSTROUTING", "-o", "eth1", "-j", "MASQUERADE"])
            subprocess.run(["ip", "route", "add", "0.0.0.0/0", "via", "192.168.70.68"])
        # Start capturing packets
        # filter_expression = "ip"
            sniff(iface="eth1", prn=packet_handler)
        else:
            print("The capture is already running.")

        return "Capture started", 200  # Return a success response
